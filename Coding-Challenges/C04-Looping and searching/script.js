document.querySelector("#search").addEventListener("click", function () {
  ...
  for (let i = 0; i < city.length; i++) {
    if (city[i].state == stateName) {
      cityName.push(city[i].name);
    }
  }
  // sending data to Html page
  for (let i = 0; i < cityName.length; i = i + 5) {
    CityNameReturn += `<tr><td>${cityName[i + 0]}</td>`;
    if (i + 1 < cityName.length) {
      CityNameReturn += `<td>${cityName[i + 1]}</td>`;
    }
    if (i + 2 < cityName.length) {
      CityNameReturn += `<td>${cityName[i + 2]}</td>`;
    }
    if (i + 3 < cityName.length) {
      CityNameReturn += `<td>${cityName[i + 3]}</td>`;
    }
    if (i + 4 < cityName.length) {
      CityNameReturn += `<td>${cityName[i + 4]}</td></tr>`;
    } else {
      break;
    }
  }
  document.querySelector("#result").innerHTML = CityNameReturn;
});
